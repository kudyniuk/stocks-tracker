import { capitalize } from "@mui/material"
import { Action, PayloadActionCreator } from "@reduxjs/toolkit"
import { openModal } from "../../../store/modal/modalSlice"
import { registerModal } from "./registerModal"

type ModalOptions<Id, Props> = {
    id: Id
    component: React.FC<Props>
}

type ModalActions = {
    close: () => void
}

export const createModal = <Id extends string, Params>({ id, component }: ModalOptions<Id, Params & ModalActions>) => {
    registerModal(id, component)
    return {
        ["open" + capitalize(id) + "Modal"]: (props: any) => openModal({modalId: id, props})
    } as unknown as Record<`open${Capitalize<Id>}Modal`, PayloadActionCreator<Params>>
}

